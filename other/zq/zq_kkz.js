...
let zq_kkzbody= $.isNode() ? (process.env.zq_kkzbody ? process.env.zq_kkzbody : "") : ($.getdata('zq_kkzbody') ? $.getdata('zq_kkzbody') : "")
let zq_kkzbodyArr = []
let zq_kkzbodys = ""
...
if (zq_kkzbody) {
    if (zq_kkzbody.indexOf("&") == -1) {
        zq_kkzbodyArr.push(zq_kkzbody)
    } else if (zq_kkzbody.indexOf("&") > -1) {
        zq_kkzbodys = zq_kkzbody.split("&")
    } else if (process.env.zq_kkzbody && process.env.zq_kkzbody.indexOf('&') > -1) {
        zq_kkzbodyArr = process.env.zq_kkzbody.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zq_kkzbody = fs.readFileSync("zq_kkzbody.txt", "utf8");
    if (zq_kkzbody !== `undefined`) {
        zq_kkzbodys = zq_kkzbody.split("\n");
    } else {
...
Object.keys(zq_kkzbodys).forEach((item) => {
    if (zq_kkzbodys[item] && !zq_kkzbodys[item].startsWith("#")) {
        zq_kkzbodyArr.push(zq_kkzbodys[item])
    }
})
...
!(async () => {
    ...
    console.log(`共${zq_kkzbodyArr.length}个看看赚body`)
	        for (let k = 0; k < zq_kkzbodyArr.length; k++) {

                zq_kkzbody1 = zq_kkzbodyArr[k];
...
//获取看看赚激活body
async function getzq_kkzbody() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zq_kkzbody) {
            if (zq_kkzbody.indexOf(bodyVal) > -1) {
                $.log("此看看赚任务请求已存在，本次跳过")
            } else if (zq_kkzbody.indexOf(bodyVal) == -1) {
                zq_kkzbodys = zq_kkzbody + "&" + bodyVal;
                $.setdata(zq_kkzbodys, 'zq_kkzbody');
                $.log(`${$.name}获取看看赚任务: 成功, zq_kkzbodys: ${bodyVal}`);
                bodys = zq_kkzbodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zq_kkzbody');
            $.log(`${$.name}获取看看赚任务: 成功, zq_kkzbodys: ${bodyVal}`);
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zq_kkzbody').replace(zq_kkzbody1 + "&", "");
                    $.setdata(smbody, 'zq_kkzbody');
