...
const $ = new Env("中青看点看视频");
const notify = $.isNode() ? require('./sendNotify') : '';
message = ""


let zq_kspbody= $.isNode() ? (process.env.zq_kspbody ? process.env.zq_kspbody : "") : ($.getdata('zq_kspbody') ? $.getdata('zq_kspbody') : "")
let zq_kspbodyArr = []
let zq_kspbodys = ""
...
if (zq_timebody) {
    ...
    if (zq_timebody !== `undefined`) {
        ...
        $.msg($.name, '【提示】请点击看视频获取timebody，再跑一次脚本', '不知道说啥好', {
            "open-url": "给您劈个叉吧"
        });
        $.done()
    }
...
if (zq_kspbody) {
    if (zq_kspbody.indexOf("&") == -1) {
        zq_kspbodyArr.push(zq_kspbody)
    } else if (zq_kspbody.indexOf("&") > -1) {
        zq_kspbodys = zq_kspbody.split("&")
    } else if (process.env.zq_kspbody && process.env.zq_kspbody.indexOf('&') > -1) {
        zq_kspbodyArr = process.env.zq_kspbody.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()) {
    var fs = require("fs");
    zq_kspbody = fs.readFileSync("zq_kspbody.txt", "utf8");
    if (zq_kspbody !== `undefined`) {
        zq_kspbodys = zq_kspbody.split("\n");
    } 
}else {
...
Object.keys(zq_kspbodys).forEach((item) => {
    if (zq_kspbodys[item] && !zq_kspbodys[item].startsWith("#")) {
        zq_kspbodyArr.push(zq_kspbodys[item])
    }
})
...
!(async () => {
    if (typeof $request !== "undefined") {
     ...
     $.done()
        ...
        console.log(`共${zq_kspbodyArr.length}个视频body`)
        for (let k = 0; k < zq_kspbodyArr.length; k++) {
            // $.message = ""
            zq_kspbody1 = zq_kspbodyArr[k];
            // console.log(`${zq_kspbody1}`)
        ...
        //     if (message.length != 0) {
        //            await notify.sendNotify("晶彩看点文章视频", `${message}\n\n shaolin-kongfu`);
        //     }
        // } else {
        //     $.msg($.name, "",  message)
        // }
...
function getzq_kspbody() {
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/article\/info.json/)||$request.url.match(/\/kandian.wkandian.com\/v5\/article\/detail.json/)) {
        ...
        if (zq_kspbody) {
            if (zq_kspbody.indexOf(bodyVal) > -1) {
                $.log("此视频请求已存在，本次跳过")
            } else if (zq_kspbody.indexOf(bodyVal) == -1) {
                zq_kspbodys = zq_kspbody + "&" + bodyVal;
                $.setdata(zq_kspbodys, 'zq_kspbody');
                $.log(`${$.name}获取视频: 成功, zq_kspbodys: ${bodyVal}`);
                bodys = zq_kspbodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个视频请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zq_kspbody');
            $.log(`${$.name}获取视频: 成功, zq_kspbodys: ${bodyVal}`);
    ...
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/user\/stay.json/)) {
        ...
        if (zq_timebody) {
            if (zq_timebody.indexOf(bodyVal) > -1) {
                ...
                $.log(`${$.name}获取视频: 成功, zq_timebodys: ${bodyVal}`);
