...
let zq_wzbody= $.isNode() ? (process.env.zq_wzbody ? process.env.zq_wzbody : "") : ($.getdata('zq_wzbody') ? $.getdata('zq_wzbody') : "")
let zq_wzbodyArr = []
let zq_wzbodys = ""
...
if (zq_wzbody) {
    if (zq_wzbody.indexOf("&") == -1) {
        zq_wzbodyArr.push(zq_wzbody)
    } else if (zq_wzbody.indexOf("&") > -1) {
        zq_wzbodys = zq_wzbody.split("&")
    } else if (process.env.zq_wzbody && process.env.zq_wzbody.indexOf('&') > -1) {
        zq_wzbodyArr = process.env.zq_wzbody.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()) {
    var fs = require("fs");
    zq_wzbody = fs.readFileSync("zq_wzbody.txt", "utf8");
    if (zq_wzbody !== `undefined`) {
        zq_wzbodys = zq_wzbody.split("\n");
    } 
}else {
...
Object.keys(zq_wzbodys).forEach((item) => {
    if (zq_wzbodys[item] && !zq_wzbodys[item].startsWith("#")) {
        zq_wzbodyArr.push(zq_wzbodys[item])
    }
})
...
!(async () => {
    if (typeof $request !== "undefined") {
     ...
     $.done()
        ...
        console.log(`共${zq_wzbodyArr.length}个阅读body`)
        for (let k = 0; k < zq_wzbodyArr.length; k++) {
            // $.message = ""
            zq_wzbody1 = zq_wzbodyArr[k];
            // console.log(`${zq_wzbody1}`)
...
function getzq_wzbody() {
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/article\/info.json/)||$request.url.match(/\/kandian.wkandian.com\/v5\/article\/detail.json/)) {
        ...
        if (zq_wzbody) {
            if (zq_wzbody.indexOf(bodyVal) > -1) {
                $.log("此阅读请求已存在，本次跳过")
            } else if (zq_wzbody.indexOf(bodyVal) == -1) {
                zq_wzbodys = zq_wzbody + "&" + bodyVal;
                $.setdata(zq_wzbodys, 'zq_wzbody');
                $.log(`${$.name}获取阅读: 成功, zq_wzbodys: ${bodyVal}`);
                bodys = zq_wzbodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个阅读请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zq_wzbody');
            $.log(`${$.name}获取阅读: 成功, zq_wzbodys: ${bodyVal}`);
