...
const $ = new Env("中青每日任务");
const notify = $.isNode() ? require('./sendNotify') : '';
message = ""

let zq_onedaybody= $.isNode() ? (process.env.zq_onedaybody ? process.env.zq_onedaybody : "") : ($.getdata('zq_onedaybody') ? $.getdata('zq_onedaybody') : "")
let zq_onedaybodyArr = []
let zq_onedaybodys = ""
...
if (!zq_onedaybody) {
 ...
 else if (zq_onedaybody.indexOf("&") == -1) {
            zq_onedaybodyArr.push(zq_onedaybody)
 }
 else if (zq_onedaybody.indexOf("&") > -1) {
            zq_onedaybodys = zq_onedaybody.split("&")
 }
 else if (process.env.zq_onedaybody && process.env.zq_onedaybody.indexOf('&') > -1) {
            zq_onedaybodyArr = process.env.zq_onedaybody.split('&');
            console.log(`您选择的是用"&"隔开\n`)
 }
 else {
            zq_onedaybodys = [process.env.zq_onedaybody]
 };
    Object.keys(zq_onedaybodys).forEach((item) => {
        if (zq_onedaybodys[item]) {
            zq_onedaybodyArr.push(zq_onedaybodys[item])
        }
    })

!(async () => {
     if (typeof $request !== "undefined") {
     await getzq_onedaybody()
     $.done()
 }else{
    console.log(`共${zq_onedaybodyArr.length}个每日任务body`)
	        for (let k = 0; k < zq_onedaybodyArr.length; k++) {

                zq_onedaybody1 = zq_onedaybodyArr[k];
...
//获取看看赚激活body
async function getzq_onedaybody() {
if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
          bodyVal=$request.body
          await $.wait(1100);
        if (zq_onedaybody) {
            if (zq_onedaybody.indexOf(bodyVal) > -1) {
                $.log("此每日任务请求已存在，本次跳过")
            } else if (zq_onedaybody.indexOf(bodyVal) == -1) {
                zq_onedaybodys = zq_onedaybody + "&" + bodyVal;
                $.setdata(zq_onedaybodys, 'zq_onedaybody');
                $.log(`${$.name}获取每日任务: 成功, zq_onedaybodys: ${bodyVal}`);
                bodys = zq_onedaybodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zq_onedaybody');
            $.log(`${$.name}获取每日任务: 成功, zq_onedaybodys: ${bodyVal}`);
            $.msg($.name, `获取第一个每日任务请求: 成功🎉`, ``)
        }
    }
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活每日任务失败')
                    smbody = $.getdata('zq_onedaybody').replace(zq_onedaybody1 + "&", "");
                    $.setdata(smbody, 'zq_onedaybody');
                    console.log("该每日任务已自动删除")
                }
            } catch (e) {
            } finally {
                resolve()
