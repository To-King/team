...
!(async () => {
  ...
  $.inviteCodes = await getAuthorShareCode('https://raw.githubusercontent.com/ugg999999/Profiles/master/shareCodes/jd_updateSmallHomeInviteCode.json')
  if (!$.inviteCodes) {
    ...
    await $.wait(1000)
    $.inviteCodes = await getAuthorShareCode('https://raw.fastgit.org/ugg999999/Profiles/master/shareCodes/jd_updateSmallHomeInviteCode.json')
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
